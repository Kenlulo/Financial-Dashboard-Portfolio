...
# HỆ THỐNG AUTO-DOWNLOADER (Chạy khi thiếu data trên máy cục bộ)
file_path = os.path.join(DATA_DIR, f"{global_ticker}_snapshot.xlsx")
if not os.path.exists(file_path):
    with st.spinner(f"Kích hoạt Auto-downloader: Đang tải dữ liệu gốc BCTC cho {global_ticker} từ Internet... (Chờ khoảng 10 giây)"):
...
if not data or data.get('IncomeStatement').empty:
    st.error(f"❌ Không tìm thấy dữ liệu Báo cáo Tài chính cho {global_ticker}. Thử lại sau.")
    st.stop()
